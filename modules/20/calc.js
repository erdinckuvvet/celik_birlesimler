...
$$
R_d = \varphi R_n = ${phi_y.toFixed(2)} \times ${(Rn / 1000).toFixed(2)}
...
`;
    ...
    // 13.4.5 — Eğilmede Akma Sınır Durumu
    (function bendingYieldCalc() {
      // --- girdiler
      const t = Number(document.getElementById('kosebent_t')?.value) || Number(document.getElementById('t')?.value) || 0;
      const ex = Number(document.getElementById('ex')?.value) || 45;          // mm
      const phi_b = Number(document.getElementById('phi_bend')?.value) || 0.90;

      // shearWidths: "40,70,70"
      const rawList = (document.getElementById('shearWidths')?.value || "")
        .split(",")
        .map(s => Number(String(s).trim()))
        .filter(v => Number.isFinite(v) && v > 0);

      // d için: son uç mesafesini ekle (e_end = e1) → 40,70,70,40
      const spansFull = rawList.slice();
      if (spansFull.length >= 2) {
        const userHasTrailing = spansFull.length >= 4 && spansFull[0] === spansFull[spansFull.length - 1];
        if (!userHasTrailing) spansFull.push(spansFull[0]);
      }
      const d = spansFull.reduce((s, w) => s + w, 0); // mm
      ...
      function pickFyFromKosebent(thk) {
        ...
        const rec = db[key];
        return Number(thk > 40 && rec.Fy2 ? rec.Fy2 : rec.Fy);
      }
      let Fy = pickFyFromKosebent(t);
      if (!Number.isFinite(Fy) || Fy === 0) Fy = 355; // emniyetli varsayılan

      // --- hesaplar
      // Dikdörtgen kol için plastik kesit modülü: Wpx = b d^2 / 4  (b = t)
      const Wpx = t * d * d / 4;                // mm^3
      const Mn_Nmm = Fy * Wpx;                  // N·mm  (MPa*mm^3 = N·mm)
      const Mn_kNmm = Mn_Nmm / 1000;            // kN·mm
      const Rn_kN = ex > 0 ? (Mn_kNmm / ex) : 0; // kN
      const Rd_kN = phi_b * Rn_kN;              // kN

      // --- özet için kaydet
      const Rd_bendYield_kN = Rd_kN.toFixed(2);
      setSummary("bendYield", Rd_bendYield_kN);


      // --- KaTeX: değişkenler
      const vars6 = String.raw`
$$
b = t = ${t}\,\text{mm},\quad d = \sum b_i^{\text{full}} = ${spansFull.join(" + ")} = ${d}\,\text{mm} \\
F_y = ${Fy}\,\text{MPa},\quad e_x = ${ex}\,\text{mm},\quad \varphi_b = ${phi_b}
$$
`;

      // --- KaTeX: işlem basamakları
      const steps6 = String.raw`
$$
\begin{aligned}
W_{px} &= \frac{b\,d^2}{4} = \frac{${t}\times ${d}^2}{4} = ${Wpx.toFixed(0)}\ \text{mm}^3 \\
M_n &= F_y\,W_{px} = ${Fy}\times ${Wpx.toFixed(0)}\times 10^{-3} = ${Mn_kNmm.toFixed(0)}\ \text{kNmm} \\
R_n &= \frac{M_n}{e_x} = \frac{${Mn_kNmm.toFixed(0)}}{${ex}} = ${Rn_kN.toFixed(2)}\ \text{kN}
\end{aligned}
$$
`;

      // --- KaTeX: sonuç
      const results6 = String.raw`
$$
R_d = \varphi_b\,R_n = ${phi_b}\times ${Rn_kN.toFixed(2)} = \mathbf{${Rd_kN.toFixed(2)}\ \text{kN}}
$$
`;

      // DOM'a yaz
      const v6 = document.getElementById('vars6');
      const s6 = document.getElementById('calcSteps6');
      const r6 = document.getElementById('results6');
      if (v6) v6.innerHTML = vars6;
      if (s6) s6.innerHTML = steps6;
      if (r6) r6.innerHTML = results6;
    })();
    ...
    (function bendingFractureCalc() {
      ...
      const tNetEl = document.getElementById('t_bend_net');
      const tNet = Number(tNetEl?.value) > 0 ? Number(tNetEl.value) : tGross;

      const ex = Number(document.getElementById('ex')?.value) || 45;          // mm
      const phi_u = Number(document.getElementById('phi_bend_u')?.value) || 0.75;
      ...
      function pickFuFromKosebent(thk) {
        ...
        if (!Number.isFinite(Number(fu)) || Number(fu) === 0) fu = Number(rec.Fu) || Number(rec.Fu2);
        return Number(fu);
      }
      let Fu = pickFuFromKosebent(tGross);
      if (!Number.isFinite(Fu) || Fu === 0) Fu = 510; // emniyetli varsayılan
      ...
      const Fy = pickFyFromKosebent(tGross);

      // --- hesaplar
      // Net plastik kesit modülü (tek kol): Wn = b_net * d^2 / 4
      const Wn = tNet * d * d / 4;          // mm^3
      const Mn_Nmm = Fu * Wn;               // N·mm
      const Mn_kNmm = Mn_Nmm / 1000;        // kN·mm
      const Rn_kN = ex > 0 ? (Mn_kNmm / ex) : 0;  // kN
      const Rd_kN = phi_u * Rn_kN;                // kN


      // Kıyas için akma taraftaki φ_y*Fy — mevcut akma bloğunla tutarlı olsun diye:
      const phi_y = Number(document.getElementById('phi_bend')?.value) || 0.90;
      const cmpLeft = phi_u * Fu;   // φ_u * F_u
      const cmpRight = phi_y * Fy;   // φ_y * F_y
      const governsText = (cmpLeft > cmpRight)
        ? String.raw`\text{Bu durumda } \varphi_u F_u > \varphi_y F_y\ \Rightarrow\ \text{eğilmede kırılma tasarım kesmesi akmadan büyüktür.}`
        : String.raw`\text{Bu durumda } \varphi_u F_u \le \varphi_y F_y\ \Rightarrow\ \text{akma daha kritik olabilir.}`;

      // --- KaTeX: değişkenler
      const vars7 = String.raw`
$$
t_\text{brüt} = ${tGross}\,\text{mm},\quad
t_\text{net} = ${tNet}\,\text{mm},\quad
d = \sum b_i^{\text{full}} = ${spansFull.join(" + ")} = ${d}\,\text{mm} \\
F_u = ${Fu}\,\text{MPa},\quad F_y = ${Number.isFinite(Fy) ? Fy : "-"}\,\text{MPa},\quad
e_x = ${ex}\,\text{mm},\quad
\varphi_u = ${phi_u},\ \varphi_y = ${phi_y}
$$
`;

      // --- KaTeX: işlem basamakları
      const steps7 = String.raw`
$$
\begin{aligned}
W_n &= \frac{b_\text{net}\,d^2}{4} = \frac{${tNet}\times ${d}^2}{4} = ${Wn.toFixed(0)}\ \text{mm}^3 \\
M_{n,u} &= F_u\,W_n = ${Fu}\times ${Wn.toFixed(0)}\times 10^{-3} = ${Mn_kNmm.toFixed(0)}\ \text{kNmm} \\
R_{n,u} &= \frac{M_{n,u}}{e_x} = \frac{${Mn_kNmm.toFixed(0)}}{${ex}} = ${Rn_kN.toFixed(2)}\ \text{kN}
\end{aligned}
$$
$$
\text{Karşılaştırma:}\quad \varphi_u F_u\ \text{vs.}\ \varphi_y F_y \quad\Rightarrow\quad
${phi_u}\cdot ${Fu} \ \text{vs.}\ ${phi_y}\cdot ${Number.isFinite(Fy) ? Fy : "-"}
$$
`;

      // --- Sonuç bölümü
      const results7 = String.raw`
$$
R_{d,u} = \varphi_u\,R_{n,u} = ${phi_u}\times ${Rn_kN.toFixed(2)} = \mathbf{${Rd_kN.toFixed(2)}\ \text{kN}}
$$
<div style="margin-top:8px; text-align:center; font-size:1.05em; color:var(--card)">
${(cmpLeft > cmpRight)
          ? 'Bu durumda φ<sub>u</sub>·F<sub>u</sub> > φ<sub>y</sub>·F<sub>y</sub> ⇒ eğilmede kırılma tasarım kesmesi akmadan büyüktür.'
          : 'Bu durumda φ<sub>u</sub>·F<sub>u</sub> ≤ φ<sub>y</sub>·F<sub>y</sub> ⇒ akma daha kritik olabilir.'
        }
</div>
`;

      // DOM'a yaz
      const v7 = document.getElementById('vars7');
      const s7 = document.getElementById('calcSteps7');
      const r7 = document.getElementById('results7');
      if (v7) v7.innerHTML = vars7;
      if (s7) s7.innerHTML = steps7;
      if (r7) r7.innerHTML = results7;
